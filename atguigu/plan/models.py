...
class TaskTurnPlan:
    ...
    def from_dict(cls, task_data: dict) -> "TaskTurnPlan":
        return cls(commands=[Command.from_dict(command_data) for command_data in task_data['commands']])
...
class TurnPlan:
    ...
    def from_dict(cls, plan_data: dict) -> "TurnPlan":
        return cls(
            task=TaskTurnPlan.from_dict(plan_data['task']) if plan_data['task'] is not None else None,
            knowledge=KnowledgeTurnPlan.from_dict(plan_data['knowledge']) if plan_data[
                                                                                 'knowledge'] is not None else None,
            chitchat=ChitchatTurnPlan() if plan_data['chitchat'] is not None else None
        )
